...
foreach ($data->{'timing_results'}->{'Item'} as $drivers){
    ...
    //Color Formatting: Best Lap Speed
    ...
    //Color Formatting: Best Turn Trap Speeds (Indy Only)
    if ($eventType == 'Indy500'){
        if (array_key_exists('Best_T1_SPD', $drivers)){
    ...
    //Color Formatting: Best No-Tow Speed (Indy Only)
    ...
    if ($eventType == 'Oval'){
        if ($eventSession == 'Race'){
            ...
            print '<tr><td>' .$position. '</td><td>' .$driverName. '</td><td>' .$carNum. '</td><td>' .$lastLapTime. '</td><td>' .$lastSpeed. '</td><td>' .$bestLapTime. '</td><td>' .$bestSpeed. '</td><td>' .$status. '</td></tr>';
        }
    }
    //Indy 500
